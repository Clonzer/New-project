...
function getNumericParam(value: string | undefined) {
...
router.get("/", async (req, res) => {
  ...
  const contestsWithDetails = await Promise.all(
    contests.map(async (contest) => {
      ...
      const prizes = Array.isArray(contest.prizes) ? (contest.prizes as ContestPrize[]) : [];
      const winners = participants.slice(0, 3).map((participant, index) => ({
        position: index + 1,
        id: participant.userId.toString(),
        username: participant.displayName || participant.username,
        score: participant.score,
        prize: prizes[index]?.title || `Top ${index + 1}`,
        avatar: participant.avatarUrl,
      }));
...
router.get("/:contestId", async (req, res) => {
  const contestId = getNumericParam(getSingleParam(req.params.contestId));
  if (!Number.isFinite(contestId)) {
    res.status(400).json({ error: "validation_error", message: "Invalid contest ID" });
    return;
  }
...
router.post("/:contestId/join", requireAuth, async (req: AuthedRequest, res) => {
  ...
  const [existing] = await db
    ...
    .where(and(
      ...
      eq(contestParticipantsTable.userId, req.auth!.userId)
    ));
...
router.post("/:contestId/entries", requireAuth, async (req: AuthedRequest, res) => {
  ...
  const [existing] = await db
    ...
    .where(and(
      ...
      eq(contestEntriesTable.userId, req.auth!.userId)
    ));
